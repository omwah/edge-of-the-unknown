...
| ID | Area | Finding | Planned work |
|---|---|---|---|
| PT-32 | Tabbed screens | ~~Computer/StarDock/Starbase tab (and Computer subtab) focus should have accent-letter hotkeys shown in the tab title; the hotkey and Enter-on-a-tab should both drop focus straight onto the tab's primary content (one step, not two). Per-tab action bindings show in the footer.~~ Implemented in `playtest: WP-PR2-01 tabbed-screen keyboard model` — underlined tab-title accelerators + Enter-to-content on StarDock/Base (`ServiceHub`) and per-category on Computer. **Deferred:** per-*subtab* single-letter hotkeys on Computer (categories only for now; subviews reachable via arrows + Enter-to-content); a few tabs whose only free in-title letter collides with an action binding (StarDock Bank, Base Hardware) stay arrow/Enter-reachable. See plan §4 WP-PR2-01. | WP-PR2-01 |
