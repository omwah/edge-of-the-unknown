...
| PT-06 | Colonist recruitment | **Partly done** in `playtest: WP-PR08 StarDock scoped service UX`: the ~~StarDock Colonists tab~~ is added (berth occupancy, incentive, recruit controls). **OUTSTANDING:** the bespoke station-concourse raster (GPT image → image-to-ANSI pipeline) is *not* done — `imagegen` was unavailable this session, so the tab ships a placeholder ASCII banner (`_CONCOURSE_ART`). Tracked in §8 (Outstanding follow-ups). | WP-PR08 |
...
| PT-23 | Computer / Avoid list | ~~Make adding a sector to the route avoid list discoverable through visible actions and Help; the current `V` workflow is not obvious.~~ Fixed in `playtest: WP-PR09 Computer ownership and route UX` — Notes-tab button + route-context V hint + Help. | WP-PR09 |
