...
| `speed` | base engine rating — splits into warp speed (`turns_per_warp`, strategic) and combat speed (the flee term, §11); on a hull with a `subsystems` block these derive from the Spindrive and Thrusters instead |
| `aspects` | per-aspect base values + caps (holds, shields, cloak, sensors) — used for hulls *without* a `subsystems` block (most NPC hulls); otherwise the slotted aspects are derived (§5.1) |
| `subsystems` | *optional* (§5.1) — per-subsystem slot layout `{slot_id, legal_components, base_component\|empty}` enabling localized component damage, field repair, and component upgrades. Present on the player starter and any hull we want to support these on |
| `armament` | list of `weapon` ids (Main Gun = a `spinal` weapon; missiles = a `homing` consumable, §5.1) |
...
### 5.1 Engine Room: Subsystems and Components
...
| `thrusters` | combat/evasion speed → the engine term in the flee roll (§11) | ≥1 burner | burners, turbines, converters, secondaries | fewer burners ⇒ lower combat speed (no burners ⇒ speed halved) ⇒ harder to flee |
...
### 5.2 Planets: Types, Ownership, and Orbital Starbases

Planets are more than colony pods and descent sites: they are **typed** (what they yield), **ownable** territory (who holds them, and therefore who defends them), and some are guarded by **orbital starbases** that reuse the engine-room component model (§5.1). This section specifies all three layers. They interlock with the alliance/territory spine (§7.3), the economy (§9), exploration (§8), and combat (§11), but the structural model lives here next to the component vocabulary the starbases borrow.
...
`yield_profile` and `habitability` values are config. Type is **band-weighted** at big bang (Core and Hub favor habitable terrestrials; outer bands skew toward uncolonizable and resource-extreme worlds), and it feeds three systems: colonist production (§9), the descent surface-site roll (§8 — a `barren` rock or `asteroid_belt` is a poor colony but can hide the best ruins), and what a world makes available for trade and barter. Uncolonizable types still have extraction value — `jovian` worlds permit **fuel-scooping** and `asteroid_belt` worlds **raw mining**, both producing without colonists (and the latter is where space-pirate NPCs base, per the manual).

#### Ownership

A planet's `owner` is one of three kinds — **unowned**, an **alliance**, or the **player** — mirroring the territorial spine of the alliance system (§7.3):

- **Core Space planets are owned by the governing alliance, automatically.** Every planet in sectors 1–10 is held by `Game.core_governing_alliance_id` (the Federation at start) and **re-keys if governance changes hands** (§7.3); Core worlds are never unowned.
- **Alliance-owned** beyond the Core — a world a bloc colonized or was seeded onto (the bloc's **home cluster** just outside the Core, §6/§7.3, plus any outward holdings). Its posture toward an entrant is read through the alliance graph: rival-bloc ships and players are enemies; members and good-standing players are welcome.
- **Player-owned** — a world the player has colonized (BNT colonization, §9) or seized. Player holdings are the deep-space footholds the exploration loop builds toward.
...
A planet may carry an **orbital starbase** (`Planet.starbase_id`) — a stationary defensive structure that reuses the engine-room component model (§5.1) **minus the mobility subsystems** (no `spindrive`, no `thrusters`: a starbase neither warps nor maneuvers) and **plus a `fusion_reactor`** that takes the Spindrive's place as the base's power plant and global-efficiency source. Its three subsystems, built from the same shared component vocabulary (§5.1), are:
...
**Operational vs. derelict is emergent, not a stored flag.** A starbase has no `condition` field: it is **operational** when its component state can power and defend it (the keystone reactor `converter` present and functional, plus minimal screens and gun) and **derelict** when broken or missing components leave it unable to — exactly the same component-health logic that already governs a battle-damaged ship (§5.1). The big bang exploits this directly: a base it creates at an **owned** planet (Core / alliance / player) is built with intact components (operational — powered, crewed, defending); a base at an **unowned, uninhabited** world is subject to an `is_derelict` roll, and one that comes up derelict is **made so by removing or damaging its components** — an abandoned holding ground down by some earlier age (a hook for the lore/discovery layer, §8). Because derelict-ness is *just missing parts*, nothing about it is special-cased: it cannot defend, and bringing it back to life is ordinary engine-room work.

**Scavenging.** From a **derelict** base — or any base the **player owns** — the player can pull installed components out of the subsystem slots into the hold, exactly the engine-room cannibalize action (§5.1), feeding the same fungible component economy as combat salvage and alien barter. A derelict is a deep-space component cache: stripping it is the quick win.
...
- **alliance-owned** base → a hostile entrant is a player aligned with one of the owner's `rival_alliance_ids` (or below an at-war standing threshold), or an NPC ship whose species is rival / at-war by inter-species relations (§7.4). Core-governor bases enforce the Core sanctuary rule (§11) through exactly this mechanism.
...
6. **Alliance home clusters.** Each non-governing alliance in the drawn cast (§7) is given a compact **home cluster** — a small interlinked group of regions (config: default 3–6 sectors, always **smaller than the 10-sector Core**) carved into the **Hub and inner-Frontier** bands, **never adjacent to the Core Space** and **never directly warp-linked to another alliance's cluster**. The sectors *between* the clusters — and between every cluster and the Core — are left as **neutral lanes**: unowned, alliance-free navigable space (the ordinary cluster/bridge graph from passes 1–2) that the player threads to pass from the Core outward **without being forced through any bloc's territory**. Each cluster's regions are tagged with their `controlling_alliance_id` (§5); its worlds are seeded **alliance-owned** and its bloc's friendly-band members placed there (step 7). A `covets_core` bloc's cluster is the staging ground for the Phase-5 seize-the-Core endgame (§7.3). The generator guarantees **at least one all-neutral path from the Core to each outer band**, so the frontier is always reachable without trespass.
7. **Populate.** StarDock (Class 9) placed 2–5 hops from the Core Space. Standard ports at ~45% sector density with the terminal-space class distribution (20/20/20/10/10/10/5/5) and initial stock `randint(200, 2000)` scaled by port size; port density thins in the outer bands (deep space is wild, not commercial). Planets seeded in ~25% of sectors, each assigned a **band-weighted `planet_type`** (§5.2: Core and Hub favor habitable terrestrials; outer bands skew toward uncolonizable and resource-extreme worlds) and an **owner** (§5.2): Core planets are owned by the initial governing alliance unconditionally, Hub planets are heavily alliance/species-owned, and the **unowned fraction rises monotonically** through Frontier → Deep → Void. **Orbital starbases** are placed per config and per each owner's `starbase_policy`: a base at an owned planet is built with intact components, while a base at an **unowned, uninhabited** world gets an `is_derelict` roll — if it comes up derelict, the generator effects that by **removing or damaging the base's components** (so a derelict is just a damaged base, trivially repaired by refilling its slots, §5.2). **Aliens:** a seeded subset is drawn from the configured species roster (Section 7) — not every roster member need appear — and each instantiated species draws its base disposition from a config-bounded spread around its roster center, then is assigned home regions. The Core Space and the **neutral lanes** around it receive only members of the initial governing alliance (the Federation by default) whose realized disposition sits in the friendly band, keeping the player's home approaches safe; each **other alliance's home cluster** (step 6) is filled with its own bloc's friendly-band members and its worlds set alliance-owned, while unaligned species — and any rival blocs' outward holdings — claim territory in the outer bands, with homeworlds at tunnel endpoints (twclone) and mean threat rating rising (and mean disposition falling) with band. Hub-band alliance clusters are populated from **friendly-band** draws like the governing alliance's, so the Hub stays innately peaceable: a cluster's danger to the player is **alliance-political** (rival-bloc standing, activated once the player aligns against it, §7.3/§11), not low disposition. **Discoveries:** every sector and planet rolls on its band's rarity table (Section 8); planets additionally roll surface sites (ruins, artifacts, ancient tech, crashed ships).
...
**Each alliance holds a home cluster.** Where the governing alliance holds the Core, every *non-governing* alliance is seeded at big bang with its own compact **home cluster** of worlds in the Hub and inner-Frontier bands (§6) — smaller than the Core, never adjacent to it, and separated from the Core and from rival clusters by **neutral navigable lanes** of unowned space. A cluster is the bloc's territorial anchor: its regions carry the bloc's `controlling_alliance_id`, its worlds are alliance-owned (§5.2) and defended by any orbital starbases there, its friendly-band members live there, and its `admission_price` tasks and rival-razing set-pieces play out around it. The neutral lanes are the player's early freedom: the frontier is always reachable by threading the empty space between clusters, so committing to no bloc never walls the player in. But because the cluster is friendly-band by *disposition*, its space is peaceful only until the player aligns against the bloc — joining a rival turns that cluster hostile on sight (§11), re-keying which near-home routes are safe. A `covets_core` bloc's home cluster is the forward base from which it can be championed into seizing the Core itself (Phase 5).

**The Federation is one alliance among many — it just starts in charge of the Core Space.** No alliance is privileged in the data model; the Federation is an ordinary `Alliance` record. What gives it its starting prominence is *governance of the Core Space* — the protected central region of sectors 1–10 (§6). Governance is a single piece of mutable game state, `Game.core_governing_alliance_id`, initialized at big bang from the roster's `initial_core_governor` setting (the Federation in the default roster). The governing alliance's friendly-band members are the species seeded into the Core and the neutral home lanes around it (every other bloc holds its own home cluster elsewhere in the Hub, §6, §7.3), its forces keep the peace there, and **the player begins as a member of that governing alliance** (`Player.alliance_id` starts as the Federation) — which is exactly what makes the Core Space a safe home to bank, trade, and resupply in at the start of a game.
...
- *Planets* — beyond their classic TW2002 colony role, planets can be **descended onto**. Surface exploration reveals sites: ruins, artifacts, ancient technology caches, crashed ships. A planet's **`planet_type` and `owner` (§5.2)** shape both its production value and its discovery value — the least habitable, unowned worlds of the deep bands (barren rocks, asteroid belts) make poor colonies but often hide the richest ruins, and a **derelict orbital starbase** is itself a discovery: a powered-down component cache to strip or repair.
...
**Planetary production and ownership.** Colonized planets produce per BlackNova's colonist model (§2.3) — colonists generate ore / organics / equipment / fighters per tick by player-set allocation sliders, with food consumption, starvation, reproduction, and interest — but the output is **shaped by `planet_type` (§5.2)**: a type's `yield_profile` multiplies its per-commodity production (a `terrestrial_hot` world pours out Fuel Ore but little else; a `terrestrial_warm` world is an Organics engine), and its `habitability` caps colonist population and growth (watery `cool` worlds sustain the largest colonies; irradiated `hot` and frozen `cold` worlds support only sparse ones). Uncolonizable types yield through extraction rather than colonists — `jovian` fuel-scooping and `asteroid_belt` raw mining — and `barren` worlds produce nothing. Only a planet's **owner** collects its production: an alliance for its holdings, the player for worlds they have **claimed** (colonizing an unowned habitable world is the player's path to a deep-space production base, §5.2). Core worlds, owned by the governing alliance, are off-limits to player colonization.
