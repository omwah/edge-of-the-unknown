...
**Phase 2 — Exploration & discovery.** The pivot phase. Discovery system with distance-banded rarity tables (wrecks, nebulae, black holes, entities), planet descent with surface sites (ruins, artifacts, ancient tech, crashed ships), sensor-based detection of hidden finds, discovery codex; friendly-disposition alien species with tech levels, contact screen, latinum sales and artifact barter for aspect upgrades (engine/shields/sensors/cloak/holds); the **engine-room subsystem/component model (§5.1)** — slotted Spindrive/Thrusters/Screens/Main Gun, the shared component vocabulary with tech tiers, derived aspects, and component install/swap upgrades bought at StarDock or bartered from aliens; StarDock (shipyard, hardware emporium, bank), multiple ship types from config, planets with BNT production model, Genesis torpedoes, Computer screen with pair-trade finder and route planner. *Exit criterion: a one-hour push-out-and-return exploration run is fun and yields tech that trading alone could not buy.*
