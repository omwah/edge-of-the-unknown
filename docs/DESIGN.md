...
│   │   ├── aliens.py         # Alien species archetypes, disposition, attitude, tech-barter offers
...
| `Player` | id, name, latinum, bank_balance, turns_remaining, alignment, experience, corp_id, explored_sectors (bitset), ship_id, species_attitudes {species_id: offset} (per-species disposition offset, applied atop base disposition), codex (found discovery ids) |
| `AlienSpecies` | id, name, archetype_id, roster_id (source roster + entry), disposition (0.0 hostile – 1.0 friendly; drawn per-generation from a spread around the roster center), federation_aligned, tech_level (1–10), home_region_id, threat_rating, interception_rating, encounter_weight |
...
Alien species rosters — names, archetypes, base disposition centers and variance, tech curves, threat/interception tables, encounter weights, barter preferences, and which roster a game uses — are config data, not code.
...
**Territory and law.** FedSpace is combat-free and deployment-free, hosts only Federation-aligned friendly-band species, and Federal response punishes criminal alignment there. Hostile-disposition species defend their home regions and occasionally raid trade lanes near the band boundary; raiding *their* homeworlds for legendary-tier technology caches is the single-player long game, with bounties per hostile fighter destroyed echoing the Cabal's 100/kill. Attacking any species' ships drives its attitude offset down — souring effective disposition and locking away tech offers — so piracy has a price even against peoples who greeted you warmly.
...
**Screen map.** `MainMenu` → `Game` (the primary screen) with modal/pushed screens: `PortScreen`, `PlanetScreen` (orbit view → `SurfaceScreen` for descent and site-by-site exploration), `StarDockScreen` (shipyard/hardware/bank/tavern tabs), `AlienContactScreen` (greeting, tech offers, latinum purchase and artifact barter), `EncounterScreen` (greeting or fight/flee rounds, depending on the species' effective disposition), `ComputerScreen`, `MapScreen`, `MessagesScreen`.
