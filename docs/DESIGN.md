...
| `Player` | id, name, latinum, bank_balance, turns_remaining, alignment, experience, corp_id, explored_sectors (bitset), ship_id, species_attitudes {species_id: offset} (per-species disposition offset, applied atop base disposition), alliance_id (joined bloc or none), alliance_standing {alliance_id: score}, grudges {species_id: Grudge} (active vendettas held against the player), codex (found discovery ids) |
...
**Territory and law.** FedSpace is combat-free and deployment-free, hosts only Federation-aligned friendly-band species, and Federal response punishes criminal alignment there. Hostile-disposition species defend their home regions and occasionally raid trade lanes near the band boundary; raiding *their* homeworlds for legendary-tier technology caches is the single-player long game, with bounties per hostile fighter destroyed echoing the Cabal's 100/kill. Attacking any species' ships drives its attitude offset down — souring effective disposition and locking away tech offers — so piracy has a price even against peoples who greeted you warmly; against a `betrayal_model=permanent` species a single betrayal floors that offset for good (§7.5).
