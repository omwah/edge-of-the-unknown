...
| `Player` | id, name, latinum, bank_balance, turns_remaining, alignment, experience, corp_id, explored_sectors (bitset), ship_id, race_attitudes {race_id: offset} (per-race disposition offset, applied atop base disposition), codex (found discovery ids) |
| `Race` | id, name, archetype_id, disposition (0.0 hostile – 1.0 friendly), federation_aligned, tech_level (1–10), home_region_id, threat_rating, interception_rating, encounter_weight |
...
6. **Populate.** StarDock (Class 9) placed 2–5 hops from FedSpace. Standard ports at ~45% sector density with the terminal-space class distribution (20/20/20/10/10/10/5/5) and initial stock `randint(200, 2000)` scaled by port size; port density thins in the outer bands (deep space is wild, not commercial). Planets seeded in ~25% of sectors. **Races:** the friendly-skewed roster (Section 7) is assigned home regions — FedSpace and all Core-band regions receive only Federation-aligned races whose disposition sits in the friendly band; lower-disposition races claim territory in outer bands, with homeworlds at tunnel endpoints (twclone) and mean threat rating rising (and mean disposition falling) with band. **Discoveries:** every sector and planet rolls on its band's rarity table (Section 8); planets additionally roll surface sites (ruins, artifacts, ancient tech, crashed ships).
7. **Validate.** Assert: single strongly-reachable component from sector 1 (treating one-ways correctly); max warps per sector ≤ 6 (TW2002 canon); StarDock reachable; at least one profitable port-pair (opposed classes, e.g. BBS↔SSB) within 5 hops of FedSpace so new players can earn; per-region port balance within tolerance; no race below the friendly disposition band present in FedSpace or Core-band regions; mean discovery rarity/value strictly increasing across bands; mean race disposition non-increasing across bands; at least one friendly-disposition contact point per band so deep explorers can resupply and barter. Regenerate with a perturbed sub-seed on failure (bounded retries, then error).
...
The universe is inhabited by alien races, generated at big bang from a config-defined roster of archetypes and seeded into regions of the warp graph. Races replace the single faceless NPC faction of classic TW2002 — the Cabal/Ferengi lineage survives as the most hostile-leaning archetypes and as the friendly traders who keep the economy circulating.

**Disposition.** A race is not simply *friendly* or *hostile*. Each race carries a **disposition** — a real number on a continuous scale from `0.0` (utterly hostile) to `1.0` (warmly friendly) — which is its innate baseline stance toward outsiders. The roster skews peaceable (default mean disposition above the midpoint, config-tunable), so amicable races outnumber dangerous ones, but most races live in the broad middle: wary, transactional peoples who will trade yet bristle when provoked. Two configurable thresholds carve the scale into descriptive bands used by generation, prose, and UI labels — a *hostility threshold* (default 0.35) below which a race reads as "hostile," and an *amity threshold* (default 0.65) above which it reads as "friendly" — but behavior interpolates continuously along the scale rather than switching at any single line. FedSpace and the Core distance band are populated exclusively by Federation-aligned races whose disposition sits in the friendly band; the likelihood of low-disposition races rises with distance from FedSpace, so genuinely hostile territory emerges in the Frontier band and deepens outward.

**Effective disposition.** A race's base disposition is adjusted per player by an *attitude offset* — raised by trading and doing favors, lowered by attacking the race's ships — yielding an **effective disposition** clamped to `[0.0, 1.0]`. Effective disposition is the single quantity that drives every interaction: the probability that an encounter opens with violence rather than greeting, the prices and barter terms a race offers, and which technology tiers it will unlock. A player can thus thaw a wary race into a trading partner, or sour a friendly one into an enemy, by their conduct — no race is permanently fixed at a binary stance.

**Tech level.** Each race has a tech level (1–10) expressed in two ways: how fast its ships travel (the engine speed of its hulls — which for hostile-leaning races also makes them harder to outrun) and what technology upgrades it can offer the player. Races disposed to deal will sell or barter upgrades to ship aspects — engine tunings, shield arrays, sensor suites, cloaking modules, hold expansions, armaments — priced in gold-pressed latinum or traded against artifacts and recovered technology from the player's discoveries. Higher-tech races offer better goods but demand rarer barter; race tech level loosely correlates with distance from FedSpace, so the best upgrades require traveling to, and surviving, deep space.

**Threat and interception.** Every race carries a *threat rating* (damage dealt per combat round) and an *interception rating* (how effectively it prevents a fleeing player from escaping), but how readily it brings them to bear scales with effective disposition: the chance an encounter turns violent rises as effective disposition falls toward `0.0`, and a warmly disposed race will almost never open fire. Among the hostile-leaning races, encounter frequency is inversely proportional to threat — the deadliest races are the rarest (encounter weights per band live in config alongside the roster). Flee resolution is specified in Section 11; as a hard invariant, the player's escape probability never drops below a configured floor (default 10%), no matter how heavily damaged the ship is.

**Attitude.** The per-player attitude offset described above is a simple scalar — raised by trading and favors, lowered by aggression — that shifts effective disposition and thereby gates access to a race's best technology tiers. Full diplomacy trees are out of scope for v1.

Race rosters — names, archetypes, base dispositions, tech curves, threat/interception tables, encounter weights, barter preferences — are config data, not code.
...
**Race encounters.** Moving through or lingering in inhabited territory rolls for encounters from the region's race table, with weights inverse to threat rating (Section 7): common raiders harass the Frontier; the apex races of the Void are rarely seen and never forgotten. Whether a given encounter opens peacefully or with violence is then rolled against the race's effective disposition (Section 7) — a high-disposition race greets the player and offers trade, a low-disposition one attacks on sight, and the wary middle is a genuine coin-flip swayed by accumulated attitude. A hostile-turning encounter opens with a detection check — the race's sensors against the player's cloak rating plus any nebula cover; an undetected player may slip away freely, making stealth a genuine alternative to firepower. Once engaged, each round the player chooses **fight** or **flee**. Flee success is a function of base chance, the player's engine speed minus the race's interception rating, cloak rating, and accumulated hull damage — and is *clamped to a configured floor (default 10%)*, so escape is always possible even in a crippled ship; this floor is a core invariant with its own property test. A failed flight attempt costs one round of incoming damage at the race's threat rating. Shields absorb damage before hull; ship destruction drops the player to an escape pod if owned.
...
**Territory and law.** FedSpace is combat-free and deployment-free, hosts only Federation-aligned friendly-band races, and Federal response punishes criminal alignment there. Hostile-disposition races defend their home regions and occasionally raid trade lanes near the band boundary; raiding *their* homeworlds for legendary-tier technology caches is the single-player long game, with bounties per hostile fighter destroyed echoing the Cabal's 100/kill. Attacking any race's ships drives its attitude offset down — souring effective disposition and locking away tech offers — so piracy has a price even against peoples who greeted you warmly.
...
**Screen map.** `MainMenu` → `Game` (the primary screen) with modal/pushed screens: `PortScreen`, `PlanetScreen` (orbit view → `SurfaceScreen` for descent and site-by-site exploration), `StarDockScreen` (shipyard/hardware/bank/tavern tabs), `RaceContactScreen` (greeting, tech offers, latinum purchase and artifact barter), `EncounterScreen` (greeting or fight/flee rounds, depending on the race's effective disposition), `ComputerScreen`, `MapScreen`, `MessagesScreen`.
...
**Phase 2 — Exploration & discovery.** The pivot phase. Discovery system with distance-banded rarity tables (wrecks, nebulae, black holes, entities), planet descent with surface sites (ruins, artifacts, ancient tech, crashed ships), sensor-based detection of hidden finds, discovery codex; friendly-disposition races with tech levels, contact screen, latinum sales and artifact barter for aspect upgrades (engine/shields/sensors/cloak/holds); StarDock (shipyard, hardware emporium, bank), multiple ship types from config, planets with BNT production model, Genesis torpedoes, Computer screen with pair-trade finder and route planner. *Exit criterion: a one-hour push-out-and-return exploration run is fun and yields tech that trading alone could not buy.*

**Phase 3 — Danger.** Low-disposition (hostile-band) races with threat/interception ratings and rarity-inverse encounter weights, the encounter system (disposition roll for greeting vs. violence, detection, fight/flee rounds, escape-probability floor), ship combat with salvage and escape pods, black hole and entity hazards live, sector fighters (off/def/toll), mines, alignment/experience, FedSpace law, attitude-offset consequences on effective disposition, friendly-disposition NPC traders moving real goods, hostile homeworld raids with bounties. *Exit criterion: the outer bands feel scary but irresistible; players quote their narrow escapes.*
