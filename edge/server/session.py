...
def _ship_dto(ship: Ship, player: Player, sector: Sector) -> dto.ShipDTO:
    aspects = [
    ...
    region_map = [f"({sector.id})"] + [
        f"  -> {t}" + ("" if t in player.explored_sectors else " ?") for t in sector.warps_out
    ]
    return dto.ShipDTO(
        ...
        latinum=player.latinum, band=sector.distance_band, region_map=region_map,
    )


def _sector_dto(state: UniverseState, player: Player, sector: Sector) -> dto.SectorDTO:
    ...
    warps = [
        dto.WarpDTO(
            sector_id=target,
            arrow=">" if target in player.explored_sectors else "?",
            explored=target in player.explored_sectors,
        )
    ...
    return dto.SectorDTO(
        ...
        beacon=sector.beacon_text, ports=ports, planets=planets, ships=[], warps=warps,
    )
...
def game_view(state: UniverseState, player_id: int, config: GameConfig) -> dto.GameState:
    ...
    return dto.GameState(
        ...
        ship=_ship_dto(ship, player, sector), sector=_sector_dto(state, player, sector),
    )
