...
def test_serve_hosts_the_plain_command() -> None:
    ...
    # The served subprocess runs the ordinary app (never --serve), so no recursion; it is
    # launched through this interpreter's own executable so a venv/pixi env is preserved.
    assert server.call_args.args[0] == f"{sys.executable} -m edge.tui --plain"
    assert server.call_args.kwargs == {
        "host": "0.0.0.0", "port": 1234,
        "title": "Edge of the Unknown", "public_url": None,
    }
...
def test_main_routes_serve_flag(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    serve.assert_called_once_with(
        "localhost", 9000, plain=False, connect=None, public_url=None)
